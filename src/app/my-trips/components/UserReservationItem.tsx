...
const UserReservationItem = ({reservation, fetchReservation}: UserReservationItemProps) => {
  ...
  const handleDeleteClick = async () => {
    ...
    if(!res){
      return toast.success('Ocorreu um erro ao cancelar a reserva!', {position: "bottom-center"})  
    }
  ...
  return (
    <div>
      <div className="flex flex-col p-5 mt-5 border-grayLighter border-solid border shadow-lg rounded-lg">
        ...
        <Button onClick={handleDeleteClick} className="mt-5" variant="danger">Cancelar</Button>
      </div>
    </div>
  </div>
  )
